...
bool vk::method::utils::check_link(std::string_view url) const {
  ...
  if (response.begin().key() == "error") { processing::log_and_throw<exception::invalid_parameter_error>("utils", response); }
...
std::string vk::method::utils::get_short_link(std::string_view url) const {
  ...
  if (response.begin().key() == "error") { processing::log_and_throw<exception::access_error>("utils", response); }
...
std::int64_t vk::method::utils::resolve_screen_name(std::string_view screen_name) const {
  ...
  if (response["response"].get_array().size() == 0) { processing::log_and_throw<exception::access_error>("utils", "No such user."); }

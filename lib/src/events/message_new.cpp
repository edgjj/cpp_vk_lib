...
vk::event::message_new::message_new(simdjson::dom::object&& event)
  ...
  , _action()
  , _attachment_handler() {
  if (get_event()["reply_message"].is_object()) _has_reply = true;

  if (get_event()["attachments"].is_array()) _has_attachments = true;

  if (get_event()["fwd_messages"].is_array() && get_event()["fwd_messages"].get_array().size() != 0) _has_fwd_messages = true;
...
void vk::event::message_new::try_get_actions() {
  ...
  if (action_name == "chat_invite_user") { _action = action::chat_invite_user{action["member_id"].get_int64()}; }
  if (action_name == "chat_kick_user") { _action = action::chat_kick_user{action["member_id"].get_int64()}; }
  ...
  if (action_name == "chat_title_update") { _action = action::chat_title_update{action["text"].get_c_str().take_value()}; }
}
